...
plane_features = torch.load('results/features/plane_features_cond.pth')
motion_features = torch.load('results/features/motion_features_cond.pth')
...
for idx in range(265):
    os.makedirs('results/mesh_cond/{}'.format(idx), exist_ok=True)
    create_mesh(sdf_decoder, 'results/mesh_cond/{}/mesh_{}_{}'.format(idx, idx, 0), 0, idx)
