...
class ConvPointnetDenseCls(nn.Module):
    def __init__(self, k, c_dim, hidden_dim, plane_resolution):
        super().__init__()
        self.k = k
        self.c_dim = c_dim
        self.hidden_dim = hidden_dim
        self.plane_resolution = plane_resolution
        self.feat = ConvPointnet(c_dim=c_dim, hidden_dim=hidden_dim, plane_resolution=plane_resolution)
        self.linear1 = torch.nn.Linear(c_dim, c_dim)
        self.linear2 = torch.nn.Linear(c_dim, c_dim // 2)
        self.linear3 = torch.nn.Linear(c_dim // 2, k)
    
    def forward(self, pc):
        c = self.feat.get_point_cloud_features(pc)
        out = F.relu(self.linear1(c))
